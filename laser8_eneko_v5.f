...
C
      ...
      write(12,1167) 
1167  format("Time (s)", 8X, "Depth (cm)", 2X, "Velocity (cm/s)", 1X,
     &      "DT (s)", 6X, "Depth1(cm)", 2X, "Depth2(cm)")
C ==========================================================================
C
C  BEGIN TIME LOOP
  27  NM1=N-1
...
C  TRIANGULAR PULSE
  87  TD = 0.25D0*TH
      IF(TIME.LE.TD) S1=2.0D0*TIME*S1/TD
      IF(TIME.GT.TD) S1=2.0D0*(S1-(TIME-TD)*S1/(PULDUR-TD))
      GO TO 86
C  USER SUPPLIED NORMALIZED PROFILE (AREA UNDER CURVE = 1)
C  THIS IS AN EXCIMER PROFILE
 89   IF(TIME.LT.40.D-9) GO TO 841
        S1=S1*PULDUR*(7.84728D7+TIME*(-2.33377D15+1.74295D22
     &                                           *TIME))
      GO TO 86
 841  IF(TIME.LT.16.D-9) GO TO 842
...
C  ESTABLISH MAXIMUM MELT-FRONT PENETRATION AND TIME
  ...
  895 CONTINUE
      ...
      write(12,1168) TIME, DEPTH, V, DT, DEP1, DEP2
 1168 format(E13.5,E13.5,E13.5, E13.5, E13.5, E13.5)
C -------------------------------------------------------------------
